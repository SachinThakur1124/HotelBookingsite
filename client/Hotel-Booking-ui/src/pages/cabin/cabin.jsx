...
export default function cabin() {
  ...
  const { data, loading } = useFetch(`/cabins/find/${id}`);
  ...
  return (
    <div>
      <Navbar />
      <Header type="list" />
      {loading ? ("Loading") : (<div className="cabinContainer">
        ...
        <div className="cabinWrapper">
          ...
          <h1 className="cabinTitle">{data.name}</h1>
          ...
          <div className="cabinImages">
            {data.photos?.map((photo, index) => (
              <div className="cabinImageWrapper">
                <img onClick={() => handleOpen(index)} src={photo} alt="" className="cabinImg" />
              </div>
            ))}
          </div>
          <div className="cabinDetails">
            <div className="cabinsDetailsTexts">
              <h1 className="cabinTitle">{data.title}</h1>
              <p className="cabinDesc">{data.desc}</p>
            </div>
            <div className="cabinsDetailsPrice">
      ...
      {openModal && <Reserve setOpen={setOpenModal} cabinId={id} />}
    </div>
  );
};
